...
export default function RightPanel({ users = [], posts = [] }) {
    ...
    return (
        <aside className="sticky top-[88px] hidden h-[calc(100vh-110px)] space-y-4 self-start overflow-y-auto pr-1 lg:block">
            <section className="rounded-3xl border border-cyan-200/10 bg-slate-900/50 p-4 backdrop-blur-xl">
                ...
                <div className="space-y-2">
                    {trendingHashtags.length === 0 ? (
                        <p className="text-sm text-slate-400">No hashtags yet.</p>
                    ) : (
                        trendingHashtags.map((item) => (
                            <Link
                                key={item.tag}
                                to={`/hashtags/${item.tag}`}
                                className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 transition hover:border-cyan-300/30 hover:bg-cyan-500/10"
                            >
            ...
            <section className="rounded-3xl border border-cyan-200/10 bg-slate-900/50 p-4 backdrop-blur-xl">
                ...
                <div className="space-y-2.5">
                    {suggestions.map((entry) => {
                        ...
                        return (
                            <div key={entry.userId} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                                ...
                                <button
                                    ...
                                    className="group relative min-w-[84px] rounded-full bg-cyan-400 px-3 py-1.5 text-xs font-semibold text-slate-950 transition hover:bg-slate-100 disabled:opacity-70"
                                >
